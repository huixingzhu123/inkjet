...
import com.zz.demo.impl.DemoServiceImpl;
...
public class DemoController {
     ...
     */
    @Logging
    @RequestMapping(value = "/demos/{systemid}", method = RequestMethod.GET)
    public ResponseEntity<Demo> getBySystemId(@PathVariable("systemid") String systemid) throws Exception {
        return ResponseEntity.ok(demoService.getBySystemid(systemid));
    }
    ...
    public ResponseEntity<PaginationSuccessDTO<Demo>> getEntices(
        ...
        return ResponseEntity.ok(demoService.getEntices(pagination, sort));
    }
     ...
     */
    @Logging
    @RequestMapping(value = "/demos", method = RequestMethod.POST, consumes = "application/json")
    public ResponseEntity save(@RequestBody @Valid Demo entity, BindingResult result) throws Exception {
        if (result.hasErrors()) {
    ...
    @Logging
    @RequestMapping(value = "/batch-demos", method = RequestMethod.POST, consumes = "application/json")
    public ResponseEntity batchSave(@RequestBody @Valid ListEntity<Demo> entityList, BindingResult result) throws Exception {
        if (result.hasErrors()) {
            throw new ValidException(result);
        }
        //检查是否包含systemid
        ...
        //创建完成返回201状态码created
        return ResponseEntity.status(HttpStatus.CREATED).body(demoService.batchSave(entityList.getList()));
    }
    ...
    @RequestMapping(value = "/demo/{systemid}", method = RequestMethod.PUT, consumes = "application/json")
    public ResponseEntity update(@PathVariable("systemid") String systemid,
                                 @RequestBody @Valid Demo entity, BindingResult result) throws Exception {
        if (result.hasErrors()) {
            throw new ValidException(result);
        }
        if (null == demoService.getBySystemid(systemid)) {
            throw new Exception("[" + systemid + "]找不到对应实体。");
        }
        entity.setSystemid(systemid);
        return ResponseEntity.ok(demoService.update(entity));
    }
     ...
     */
    @Logging(changeObject = Demo.class)
    @RequestMapping(value = "/patch-demo", method = RequestMethod.PUT,consumes = "application/sino-patch")
    public ResponseEntity patchUpdate(@RequestBody @Valid Demo entity, BindingResult result) throws Exception {
        if (result.hasErrors()) {
            throw new ValidException(result);
        }
        return ResponseEntity.ok(demoService.update(entity));
    ...
    public ResponseEntity logicRemove(@PathVariable("ids") String ids) throws Exception {
        demoService.logicRemove(ids.split(Constants.SPLIT_COMMA));
        //返回204
